		...
		for (i = 0; i < get_clk_sources_size(); i++) {
			if (clk_srcs[i].name == NULL) {
				...
				len += seq_printf(m, "%2d %-10s %4d.%03dMHz %3sable   %d %s\n", i, clk_srcs[i].name,
						  mhz / 1000, mhz % 1000,
						  clk_srcs[i].flags & CLK_FLG_ENABLE ? "en" : "dis", clk_srcs[i].count,
...
struct cpufreq_frequency_table *init_freq_table(unsigned int max_freq, unsigned int min_freq) {
	int i, j = 0;
	...
	freq_table = (struct cpufreq_frequency_table *)kzalloc(sizeof(struct cpufreq_frequency_table) * num, GFP_KERNEL);
	if (!freq_table) {
...
static void init_freq_udelay_jiffys(struct clk *cpu_clk, unsigned int num) {
	...
	for (i = 0; i < num; i++) {
		...
		freq_udelay_jiffys[i].udelay_val = cpufreq_scale(cpu_data[0].udelay_val, rate, set_cpu_freqs[i]);
		freq_udelay_jiffys[i].loops_per_jiffy = cpufreq_scale(loops_per_jiffy, rate, set_cpu_freqs[i]);
	}
	...
	for (i = 0; i < num; i++) {
		printk("%u\t %u\t %u\t\n", freq_udelay_jiffys[i].cpufreq,
		       freq_udelay_jiffys[i].udelay_val,
		       freq_udelay_jiffys[i].loops_per_jiffy);
	}
}
...
static int __init init_clk_proc(void) {
	...
	freq_udelay_jiffys = (struct freq_udelay_jiffy *) kzalloc(sizeof(struct freq_udelay_jiffy) * num, GFP_KERNEL);
